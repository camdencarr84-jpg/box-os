...
`box-shell` is a single long-running [Quickshell](https://quickshell.org/)
...
Box is compatible with Omarchy plugins
...
| `bar`        | A full bar option that can replace the built-in `box.bar` |

Only one `bar` plugin is active at a time. Missing or invalid selections fall
back to the built-in `box.bar`, so users always have a safe path home.
...
changed bar widget cannot destroy `box.lock` while Hyprland still
...
```bash
box plugin add https://github.com/acme/box-weather.git --enable --yes
box plugin update --yes
```
...
1. Put it in `~/.config/box/plugins/<plugin-id>/` with a `manifest.json`
   plus the QML referenced from its `entryPoints`.
2. `box-shell shell rescanPlugins`.
3. `box plugin enable <id>`. Bar widgets start in
   `barWidget.defaultSection`, or in the center when it is omitted, and can be
   moved with `box bar move`; a full bar replaces the one in use.

The lower-level IPC equivalents remain available via `box-shell shell rescanPlugins`,
`box-shell shell enablePlugin <id> '{}'`, and `box-shell shell listPlugins`.
The `box plugin` commands wrap those calls. `box bar move` and
`box bar set` edit the persisted widget layout in `shell.json`.
...
Saving a file anywhere under `~/.config/box/plugins/` reloads plugin code
automatically; `box-shell shell rescanPlugins` remains available to force a reload.
...
hooks, the image picker's `image-selector` target). `box-menu` uses the
shell target to summon the first-party `box.menu` plugin instead of
...
| `reloadConfig`                           | `ok`    | reload `~/.config/box/shell.json`                 |
...
$OMARCHY_PATH/shell`. Use `box-restart-shell` to stop every running
instance of that config and launch one fresh shell process.

A convenience wrapper, [`box-shell`](../bin/box-shell), forwards IPC
calls to the running shell. It does not start the shell.

```
box-shell shell ping
box-shell shell toggle box.menu '{"menu":"root"}'
box-shell shell listPlugins
box-shell shell rescanPlugins
```
...
| Path                              | Owner          | Purpose                                                |
|-----------------------------------|----------------|--------------------------------------------------------|
| `~/.config/box/shell.json`    | the shell      | full layout + per-entry settings + enabled plugin list |
| `~/.config/box/plugins/<id>/` | user           | drop-in third-party plugin source files                |

The `config/box/shell.json` default config describes the
...
```json
{
  ...
  "bar": {
    ...
    "position": "top",
    "transparent": false,
    "centerAnchor": "box.clock",
    "layout": {
      "left":   [ { "id": "box.menu" }, { "id": "box.workspaces" } ],
      "center": [ { "id": "box.clock", "format": "HH:mm" } ],
      "right": [
        { "id": "box.audio" }
      ]
    }
...
1. **The active bar option is `bar.id`.** Omit it or set it to `box.bar`
...
4. **Built-in widget ids are namespaced.** Use ids such as `box.clock`,
   `box.audio`, and `box.network`. The migration rewrites older ids
...
6. **Multiple instances** are allowed when a manifest sets
   ...
   widgets in different timezones are just two `{"id":"box.clock", "timezone": ...}`
...
- Phase 1 — `box-shell phase 1: host the existing bar in a single shell`
- Phase 2 — `box-shell phase 2: plugin registry and bar widget registry`
- Phase 3 — `box-shell phase 3: fold bar-settings into the shell as a panel plugin`
- Phase 4 — `box-shell phase 4: absorb background-switcher as a plugin`
- Phase 5 — `box-shell phase 5: docs, cleanup, and migration crumbs`
- Phase 6 — `box-shell phase 6: reviewer cleanup (path traversal, collision, races)`
- Phase 7 — `box-shell phase 7: replace socket with IpcHandler, rename to image-picker`
- Phase 8a — `box-shell phase 8a: unified shell.json with inline plugin settings`
